...
import '../custom_bottom_picker.dart';
...
Future<T?> showPlatformDialog<T>({
  ...
  if (isMaterial(context)) {
    assert(
      material?.builder != null || builder != null,
      'This is an example of an assert message.',
    );
    ...
    assert(
      cupertino?.builder != null || builder != null,
      'This is an example of an assert message.',
    );
...
Future<T?> showPlatformModalSheet<T>({
  ...
  required Widget Function(BuildContext) cupertinoBuilder,
...
Future<DateTime?> showPlatformDatePicker<T>({
  required BuildContext context,
}) async {
  if (isMaterial(context)) {
    ...
    await showCupertinoModalPopup<DateTime>(
      context: context,
      builder: (context) => CustomBottomPicker(
...
Future<TimeOfDay?> showPlatformTimePicker<T>({
  required BuildContext context,
}) async {
  if (isMaterial(context)) {
    ...
    await showCupertinoModalPopup<DateTime>(
      ...
      builder: (context) => CustomBottomPicker(
        child: CupertinoDatePicker(
          onDateTimeChanged: (DateTime newDate) {
            // Handle date changes
            timeofDay = TimeOfDay(hour: newDate.hour, minute: newDate.minute);
          },
          minimumDate: DateTime.now(),
